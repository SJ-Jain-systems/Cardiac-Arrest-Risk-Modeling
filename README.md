...
├── LICENSE
├── Makefile
├── pyproject.toml            # source of truth for dependencies + tooling config
...
│   └── processed/            # generated; regenerated by src/create_processed_dataset.py
├── models/
...
│       ├── pipeline.py
│       ├── cli.py
│       └── test_rag_pipeline.py       # offline RAG tests (stub embedder + mocked client)
└── tests/
...
3. Install dependencies. Versions are pinned for reproducibility, with
   ...
   development tooling (`pytest`, `ruff`, `black`, `pre-commit`, `nbstripout`):
...
4. (Optional) Enable the pre-commit hooks so notebooks are committed
   output-free and code is auto-formatted:
...
| `models/best_model_metadata.json` | `notebooks/05_predictive_modeling.ipynb` | Metadata describing the selected model (committed; the `.joblib` binary is git-ignored). |
